...
- **Ignore autogenerated files**: When Copilot generates PR review suggestions, do not propose changes to
  autogenerated files (for example migration files under `migrations/` or `src/migrations/`, or any
  other generated artifacts). Focus review feedback on human-written source code and configuration.
